...
This module is designed to easily spin up a google cloud project infrastructure in a configuration that is suitable for typical gnomAD activities.

## Inventory

- Use an existing google project
...
- The user that runs the `terraform apply` that actually creates the project will be granted the Owner role.
  ...
  - You must specify a service account which has 'Editor' role. Ensure that more than one person is an owner of the project. This is to prevent a single user from being the only one who can admin the project.
  - We recommend getting a copy of the keys for this service account and authorising as this user with gcloud - rather than using your personal credentials.
...
| [google_project_iam_member.hail_batch_service_usage](https://registry.terraform.io/providers/hashicorp/google/latest/docs/resources/project_iam_member) | resource |
...
| <a name="input_default_resource_region"></a> [default\_resource\_region](#input\_default\_resource\_region) | For managed items that require a region/location | `string` | `"australia-southeast1"` | no |
...
| <a name="input_project_id"></a> [project\_id](#input\_project\_id) | The unique id of the project. | `string` | n/a | yes |
